      ...
      class Forms
        ...
        def emit(buffer, type)
          ...
          buffer.nest("module Form") do
            ...
            buffer.line("extend ::Oapi::Form::Contract")
            buffer.blank
            buffer.line("Value = type_template { { fixed: #{qualified} } }")
            buffer.blank
            buffer.line("sig { override.params(parts: ::Oapi::Form::Parts).returns(#{qualified}) }")
        ...
        def decode_extra(type, schema)
          ...
          "::Oapi::Decode.values(parts.except(#{known})) { |item| #{inner} }"
        end
      end
    end
  end
