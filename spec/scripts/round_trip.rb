...
mod = KitchenSink::Codecs::Mod::INSTANCE.from_wire(WIRE)
...
other = KitchenSink::Codecs::Mod::INSTANCE.from_wire(WIRE)
...
check("differing values are unequal") { KitchenSink::Codecs::Mod::INSTANCE.from_wire(WIRE.merge("id" => 8)) != mod }

wire = KitchenSink::Codecs::Mod::INSTANCE.to_wire(mod)
...
cat = KitchenSink::Codecs::Pet::INSTANCE.from_wire({ "kind" => "cat", "lives" => 9 })
check("discriminated union") { cat.is_a?(KitchenSink::Types::Cat) }
check("union dump") { KitchenSink::Codecs::Pet::INSTANCE.to_wire(cat)["kind"] == "cat" }

check("untagged union string") { KitchenSink::Codecs::Loose::INSTANCE.from_wire("x") == "x" }
check("untagged union integer") { KitchenSink::Codecs::Loose::INSTANCE.from_wire(3) == 3 }

begin
  KitchenSink::Codecs::Mod::INSTANCE.from_wire(WIRE.merge("id" => "not a number"))
...
begin
  KitchenSink::Codecs::Mod::INSTANCE.from_wire(WIRE.merge("tags" => ["a", 2]))
  raise "failed: expected a DecodeError"
rescue Oapi::DecodeError => e
