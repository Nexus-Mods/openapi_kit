...
RSpec.describe OpenAPIKit::Codec::Contract do
  # A type from a gem you do not control, with nothing of openapi_kit's mixed into it.
  let(:money) do
    Class.new do
      attr_reader :cents
  ...
  describe "a codec with no state" do
    let(:money_codec) do
      Module.new do
        ...
        def self.from_wire(value) = Money.new(OpenAPIKit::Codec::Integer.from_wire(value))
        def self.to_wire(value) = value.cents
      end
    end

    it "converts a type it does not own" do
      expect(money.ancestors.map(&:to_s).grep(/OpenAPIKit/)).to be_empty
    ...
    it "is how every built-in codec is written" do
      [OpenAPIKit::Codec::DateTime, OpenAPIKit::Codec::Decimal, OpenAPIKit::Codec::Uuid].each do |codec|
  ...
  describe "a codec that needs state" do
    let(:salted_id_codec) do
      Class.new do
        ...
        def from_wire(value) = OpenAPIKit::Codec::String.from_wire(value).delete_prefix(@salt).to_i
